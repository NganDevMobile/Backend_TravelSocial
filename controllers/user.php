...
class UserController {
        ...
        public function register($email, $password, $confirm_password){
            ...
            $status = $this->user_service->register($email, $hashPassword);
            return $status;
        }
        ...
        private function sendEmail ($email, $token){
            $link = "<a href='http://127.0.0.1:8088/views/user_reset_password_form.php?email="
            ...
            $mail->isSMTP();
            $mail->SMTPAuth = true;
            $mail->Username = "kietdev.test"; //cái phần trước @gmail.com
            $mail->Password = "abcabc250250"; // pass email
            $mail->SMTPSecure = "ssl";
            $mail->Host = "ssl://smtp.gmail.com";
            $mail->Port = "465";
            $mail->From = "kietdev.test@gmail.com"; //email 
            $mail->FromName = "Hiếu Ngân"; // Người gửi(để sao cũng được)
        ...
        public function updatePasswordAndToken($token, $email, $password)
        {
            ...
            if ($check_token) {
                $hashPassword = password_hash($password, PASSWORD_BCRYPT);
                $change_password = $this->user_service->changePassword($email, $hashPassword);
                if ($change_password) {
        ...
        {
            $hashPassword = password_hash($password, PASSWORD_BCRYPT);
            return $this->user_service->changePassword($email, $hashPassword);
        }
        
        public function getUserInfo(){
            return $this->user_service->getUserInfo();
        }
    }
