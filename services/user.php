<?php
    ...
    class UserService {
        ...
        public function getByEmail($email){
            try {
                $q = "SELECT id, email, hashPassword from " . $this->userAccount . " 
                ...
                if ($stmt->rowCount() > 0) {
                    ...
                    extract($row);
                    return new User($id, $email, $hashPassword);
                    // return new User($id, $email, $hashPassword, $fullname, $phone, $gender, $avatar);
                }
            } catch (Exception $e) {
                echo $e->getMessage();
            }
        ...
        public function register($email, $hashPassword)
        {
            try {
                $q = "insert into " . $this->userAccount . "
                        set email=:email,
        ...
        // change password
        public function changePassword($email, $hashPassword)
        {
            try {
                $q = "update " . $this->userAccount . "
